...
public interface TetherService {
    ...
    // 특정 지갑의 최근 입금 1건 조회
    TetherDeposit getLatestDeposit(String tetherWallet);

    // 특정 기간의 전체 입금 목록
    List<TetherDeposit> getDepositsInRange(LocalDateTime start, LocalDateTime end);

    // 특정 기간의 지갑 입금 목록
    List<TetherDeposit> getDepositsInRangeByTetherWallet(String tetherWallet, LocalDateTime start, LocalDateTime end);

...
public class ReportServiceImplements implements ReportService {
  ...
  @Override
  public Page<ReportDTO> findReportsByLevel(Level currentLevel, Long employeeId,
                                            LocalDateTime start, LocalDateTime end,
                                            Pageable pageable) {
    if (currentLevel == Level.STAFF) {
      return reportRepository.findByEmployee_idAndInsertDateTimeBetween(employeeId, start, end, pageable).map(this::toReportDto);
    }
    ...
    List<Report> reports = reportRepository.findReportsByLevelWithPaging(
            currentLevel.name(), start, end, limit, offset
    );

    List<ReportDTO> content = reports.stream()
  ...
  public Page<ReportDTO> findReportsByEmployeeName(
    ...
    long totalCount = reportRepository.countReportsByMaxLevelAndNameAndPeriod(level, employeeName.trim(), start, end);
    ...
    List<Report> reports = reportRepository.findReportsByLevelAndEmployeeNameWithPaging(
